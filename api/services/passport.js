 ...
 *
 *   1. Given a provider and an identifier, find a mathcing Passport.，提供认证服务方和身份标志，寻找匹配的passport
 *   2. From here, the logic branches into two paths.
 *
 *     - A user is not currently logged in:
 *       1. If a Passport wassn't found, create a new user as well as a new
 *          Passport that will be assigned to the user.未找到passport，则创建一个新的
 ...
 *
 * As you can see, this function handles both "authentication"认证 and "authori-
 * zation"授权 at the same time. This is due to the fact that we pass in
...
passport.callback = function(req, res, next) {
    ...
    if (provider === 'local' && action !== undefined) {
        if (action === 'register' && !req.user) {
            console.log('New User Registered ,here is passport.js line 198');
            this.protocols.local.register(req, res, next);
        } else if (action === 'connect' && req.user) {
            this.protocols.local.connect(req, res, next);
        } else if (action === 'reset' && !req.user) {
            console.log('reset password from services/passport' + req.param('email') + req.param('username'));
            this.protocols.local.reset(req, res, next);
        } else if (action === 'passwd' && req.user) {
            console.log('change password from services/passport' + req.param('identifier'));
            this.protocols.local.passwd(req, res, next);
        } else if (action === 'activate' && !req.user) {
            this.protocols.local.activate(req, res, next);
        } else {
